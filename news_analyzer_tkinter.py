...
def check_fact(claim):
    """Check a claim using Gemini first, fallback to Wikipedia if quota exceeded"""
    try:
...
CONFIDENCE: [HIGH/MEDIUM/LOW]"""
            ...
            # If Gemini fails (quota exceeded, etc), fallback to Wikipedia
...
class NewsAnalyzerApp:
    ...
    def setup_ui(self):
        ...
        factcheck_btn = tk.Button(search_inner, text="✓ FACT-CHECKER", command=self.open_factchecker, 
                                 bg="#ff6b6b", fg="#1e1e1e", font=("Helvetica", 11, "bold"), 
                                 padx=15, pady=8, relief=tk.FLAT, cursor="hand2", 
                                 activebackground="#ff8787", activeforeground="#1e1e1e")
        factcheck_btn.pack(side=tk.LEFT, padx=(12, 0))
    ...
    def open_factchecker(self):
        """Open fact-checker window"""
        fact_window = tk.Toplevel(self.root)
        fact_window.title("Fact-Checker")
        fact_window.geometry("900x700")
        fact_window.configure(bg="#1e1e1e")
        
        # Header
        header = tk.Frame(fact_window, bg="#242424", height=60)
        header.pack(fill=tk.X)
        header.pack_propagate(False)
        
        accent = tk.Frame(header, bg="#ff6b6b", height=2)
        accent.pack(fill=tk.X)
        
        title = tk.Label(header, text="✓ FACT-CHECKER", font=("Helvetica", 18, "bold"), 
                        bg="#242424", fg="#ff6b6b")
        title.pack(pady=10)
        
        # Input section
        input_frame = tk.Frame(fact_window, bg="#1e1e1e")
        input_frame.pack(fill=tk.X, padx=30, pady=20)
        
        tk.Label(input_frame, text="Enter a statement or claim to verify:", 
                font=("Helvetica", 11, "bold"), bg="#1e1e1e", fg="#6ec46d").pack(anchor="w", pady=(0, 8))
        
        # Input box with scrollbar
        input_box_frame = tk.Frame(input_frame, bg="#2d2d2d", relief=tk.FLAT, bd=1, highlightthickness=2, 
                                  highlightbackground="#5b8fc4", highlightcolor="#ff6b6b")
        input_box_frame.pack(fill=tk.X)
        
        scrollbar = tk.Scrollbar(input_box_frame)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        claim_text = tk.Text(input_box_frame, height=6, font=("Helvetica", 10), 
                           bg="#2d2d2d", fg="#e1e8ed", yscrollcommand=scrollbar.set, 
                           relief=tk.FLAT, bd=0, padx=10, pady=10, wrap=tk.WORD, insertbackground="#5b8fc4")
        claim_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.config(command=claim_text.yview)
        
        claim_text.insert(1.0, "e.g., The Earth is flat")
        
        # Button frame
        button_frame = tk.Frame(fact_window, bg="#1e1e1e")
        button_frame.pack(fill=tk.X, padx=30, pady=10)
        
        check_btn = tk.Button(button_frame, text="⚡ CHECK FACT", 
                             command=lambda: self._perform_factcheck(claim_text.get(1.0, tk.END), fact_window, results_frame),
                             bg="#ff6b6b", fg="#1e1e1e", font=("Helvetica", 11, "bold"),
        ...
        clear_btn = tk.Button(button_frame, text="CLEAR", 
                             command=lambda: claim_text.delete(1.0, tk.END),
                             bg="#2d2d2d", fg="#5b8fc4", font=("Helvetica", 10, "bold"),
        ...
        # Results section
        results_label = tk.Label(fact_window, text="FACT-CHECK RESULTS", 
                               font=("Helvetica", 11, "bold"), bg="#1e1e1e", fg="#5b8fc4")
        results_label.pack(anchor="w", padx=30, pady=(15, 10))
        
        results_frame = tk.Frame(fact_window, bg="#1e1e1e")
        results_frame.pack(fill=tk.BOTH, expand=True, padx=30, pady=(0, 20))
        
        # Canvas with scrollbar for results
        results_canvas = tk.Canvas(results_frame, bg="#1e1e1e", highlightthickness=0, bd=0)
        results_scrollbar = ttk.Scrollbar(results_frame, orient="vertical", command=results_canvas.yview)
        scrollable_results = tk.Frame(results_canvas, bg="#1e1e1e")
        ...
        self.results_container = scrollable_results
        
        # Initial placeholder
        placeholder = tk.Label(scrollable_results, text="Enter a statement and click 'CHECK FACT' to see results", 
                             font=("Helvetica", 10), fg="#5b8fc4", bg="#1e1e1e", wraplength=700)
        placeholder.pack(pady=40)
    
    def _perform_factcheck(self, claim, fact_window, results_frame):
        """Perform fact-check in background thread"""
        claim = claim.strip()
        if not claim or claim == "e.g., The Earth is flat":
            messagebox.showwarning("Input Required", "Please enter a statement to check")
            return
        ...
        loading = tk.Label(self.results_container, text="⏳ CHECKING FACT... PLEASE WAIT", 
                          font=("Helvetica", 11, "bold"), fg="#ff6b6b", bg="#1e1e1e")
        loading.pack(pady=40)
        self.root.update()
        
        thread = threading.Thread(target=self._factcheck_thread, args=(claim,))
        thread.daemon = True
        thread.start()
    
    def _factcheck_thread(self, claim):
        """Background thread for fact-checking"""
        results, error = check_fact(claim)
        
        # Clear results container
        self.root.after(0, lambda: self._clear_results_container())
        
        if error:
            error_label = tk.Label(self.results_container, text=f"❌ {error}", 
                                  font=("Helvetica", 10), fg="#ff6b6b", bg="#1e1e1e", wraplength=700)
            self.root.after(0, lambda: error_label.pack(pady=20, padx=10, fill=tk.X))
            return
        
        if not results:
            no_results = tk.Label(self.results_container, text="No fact-check information found.", 
                                 font=("Helvetica", 10), fg="#5b8fc4", bg="#1e1e1e", wraplength=700)
            self.root.after(0, lambda: no_results.pack(pady=20, padx=10, fill=tk.X))
            return
        
        # Display results
        for result in results:
            self.root.after(0, lambda r=result: self._display_factcheck_result(r))
    
    def _clear_results_container(self):
        """Clear all widgets from results container"""
        for widget in self.results_container.winfo_children():
            widget.destroy()
    
    def _display_factcheck_result(self, result):
        """Display a single fact-check result"""
        # Container for each result
        result_card = tk.Frame(self.results_container, bg="#2d2d2d", relief=tk.FLAT, bd=0)
        result_card.pack(fill=tk.X, pady=(0, 12))
        
        inner = tk.Frame(result_card, bg="#242424", padx=15, pady=12)
        inner.pack(fill=tk.BOTH, expand=True)
        
        # Claim
        claim_label = tk.Label(inner, text=f"Claim: {result['claim']}", 
                             font=("Helvetica", 10, "bold"), fg="#e1e8ed", bg="#242424",
                             wraplength=650, justify=tk.LEFT)
        claim_label.pack(anchor="w", pady=(0, 8))
        ...
        rating_color = rating_colors.get(rating, "#5b8fc4")
        
        rating_label = tk.Label(inner, text=f"✓ Verdict: {rating}", 
                              font=("Helvetica", 11, "bold"), fg=rating_color, bg="#242424")
        rating_label.pack(anchor="w", pady=(0, 8))
        
        # Explanation
        explanation = result.get("explanation", "No explanation available")
        explanation_label = tk.Label(inner, text=explanation, 
                                    font=("Helvetica", 9), fg="#a0a8b0", bg="#242424",
                                    wraplength=650, justify=tk.LEFT)
        explanation_label.pack(anchor="w", pady=(0, 8))
        
        # Confidence
        confidence = result.get("confidence", "UNKNOWN")
        confidence_label = tk.Label(inner, text=f"Confidence: {confidence}", 
                                   font=("Helvetica", 8), fg="#6ec46d", bg="#242424")
